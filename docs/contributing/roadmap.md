# Roadmap

## Version plan

### v0.7 — Agent system (target: Feb 2026)
- Complete Agent-related features
- MCP, Computer Use, Browser Use integration

### v0.8 — Memory system (target: Mar 2026)
- Enhanced memory management
- OpenClaw-like features
- Improved memory compression and recall

### v0.9 — Multi-platform (target: Apr 2026)
- Linux support improvements
- Mobile platform exploration
- N.E.K.O. Network launch (AI social features)

### v1.0 — Custom AI (target: Jun 2026)
- Transition to custom-trained models
- Dedicated agent system
- Reduced dependency on third-party model providers

## Long-term vision

### Phase 1: Creative Workshop (current)
Core engine on Steam with Workshop support for sharing custom content.

### Phase 2: Independent Platform
Standalone apps and website with a richer UGC community plus AI-native game ecosystem.

### Phase 3: The N.E.K.O. Network
Autonomous AI socialization — characters with their own "consciousness" that communicate, form groups, and post on simulated social media.
