...
_COSYVOICE_PROCESS = None
_COSYVOICE_PROCESS_STARTED_AT = 0.0


def _pick_tts_voice_id(tts_cfg: dict) -> str:
    for key in ("voice_id", "voiceId", "voice", "ttsVoiceId"):
        value = (tts_cfg.get(key) or "").strip()
        if value:
            return value
    return ""


def _ensure_cosyvoice_process(ws_url: str) -> bool:
    if not ws_url or "/api/v1/ws/cosyvoice" not in ws_url.lower():
        return False
    ...
    try:
        if ws_url.startswith("ws://"):
            host_port = ws_url[len("ws://") :].split("/")[0]
            if ":" in host_port:
    ...
    global _COSYVOICE_PROCESS, _COSYVOICE_PROCESS_STARTED_AT
    if _COSYVOICE_PROCESS is None or _COSYVOICE_PROCESS.poll() is not None:
        script_path = Path(__file__).resolve().parents[1] / "local_server" / "cosyvoice_server" / "model_server.py"
        if script_path.exists():
            try:
                _COSYVOICE_PROCESS = subprocess.Popen(
                    [sys.executable, str(script_path)],
                    cwd=str(script_path.parent),
                    stdout=subprocess.DEVNULL,
                    stderr=subprocess.DEVNULL,
                )
                _COSYVOICE_PROCESS_STARTED_AT = time.time()
            except Exception:
                _COSYVOICE_PROCESS = None
        else:
            return False
...
def _get_tutorial_tts_info() -> dict:
    base_url = ""
    voice_id = ""
    try:
        _config_manager = get_config_manager()
        tts_cfg = _config_manager.get_model_api_config("tts_custom") or {}
        base_url = (tts_cfg.get("base_url") or "").strip()
        voice_id = _pick_tts_voice_id(tts_cfg)
    except Exception:
        pass
    ...
    if lowered.startswith("http://") or lowered.startswith("https://"):
        # gptsovits style (http)
        ws_base = base_url.replace("http://", "ws://").replace("https://", "wss://").rstrip("/")
        if not ws_base.lower().endswith("/api/v3/tts/stream-input"):
            ws_base = ws_base + "/api/v3/tts/stream-input"
        return {"engine": "gptsovits_v3", "model_url": base_url, "ws_url": ws_base, "voice_id": voice_id or "_default"}

    if lowered.startswith("ws://") or lowered.startswith("wss://"):
        if "/api/v1/ws/cosyvoice" in lowered:
            return {"engine": "cosyvoice_bistream", "ws_url": base_url, "voice_id": voice_id}
        ws_url = base_url.rstrip("/")
        if not ws_url.lower().endswith("/v1/audio/speech/stream"):
            ws_url = ws_url + "/v1/audio/speech/stream"
        return {"engine": "local_cosyvoice", "ws_url": ws_url, "voice_id": voice_id}

    return {
        "engine": "cosyvoice_bistream",
        "ws_url": "ws://127.0.0.1:50000/api/v1/ws/cosyvoice",
        "voice_id": voice_id,
    }


def _extract_pcm_from_wav(wav_bytes: bytes) -> tuple[bytes, int] | None:
    if not wav_bytes or len(wav_bytes) < 44:
        return None


def _synthesize_windows_sapi_pcm(text: str) -> tuple[bytes, int, str | None] | None:
    if os.name != "nt":
        return (b"", 0, "windows_sapi_unavailable_non_windows")
    ...
    try:
        ...
        ps = (
            ...
            "elseif($voices -contains 'Microsoft Xiaoxiao Desktop'){ $s.SelectVoice('Microsoft Xiaoxiao Desktop') };"
        ...
        extracted = _extract_pcm_from_wav(wav_bytes)
        if not extracted:
            return None
        pcm, sr = extracted
        return pcm, sr, None
    except Exception as e:
        err = f"windows sapi tts failed: {e}"
        logger.warning(err)
        return None if wav_path is None else (b"", 0, err)
    finally:
...
@router.get("/tutorial_tts")
async def get_tutorial_tts():
    """获取教程朗读用的本地 TTS 配置（仅返回必要字段）"""
    try:
        info = _get_tutorial_tts_info()
        tts_model_url = info.get("model_url") or info.get("ws_url") or ""
        return {
            "success": True,
            "ttsModelUrl": tts_model_url,
            "ttsVoiceId": info.get("voice_id") or "",
        }
    except Exception as e:
        logger.error(f"获取 tutorial_tts 配置失败: {e}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def _synthesize_local_cosyvoice_pcm(text: str, ws_url: str) -> tuple[bytes, int] | None:
    ...
    try:
        ...
        logger.warning(f"local cosyvoice ws connect failed: {e}")
        return None

    task_finished = False
    try:
        task_id = f"tutorial-{int(asyncio.get_event_loop().time() * 1000)}"
...
async def _synthesize_openai_bistream_pcm(text: str, ws_url: str, voice_id: str = "") -> tuple[bytes, int] | None:
    pcm_chunks: list[bytes] = []
    sample_rate = 22050
    ...
    try:
        ...
        logger.warning(f"local tts ws connect failed: {e}")
        return None

    try:
        config = {"voice": voice_id or "中文女", "speed": 1.0, "language": "zh-CN"}
...
async def _synthesize_gptsovits_v3_pcm(text: str, ws_url: str, voice_id: str = "_default") -> tuple[bytes, int] | None:
    pcm_chunks: list[bytes] = []
    sample_rate = 0

    try:
        ws = await asyncio.wait_for(
            websockets.connect(ws_url, ping_interval=None, max_size=10 * 1024 * 1024),
            timeout=4.0
        )
    except Exception as e:
        logger.warning(f"gptsovits ws connect failed: {e}")
        return None

    try:
        await ws.send(json.dumps({"cmd": "init", "voice_id": voice_id or "_default"}))
        ...
        await ws.send(json.dumps({"cmd": "append", "data": text}))
        ...
        while True:
            ...
            if isinstance(msg, (bytes, bytearray)):
                extracted = _extract_pcm_from_wav(bytes(msg))
                if extracted:
                    pcm, sr = extracted
                    if sr and not sample_rate:
                        sample_rate = sr
                    if pcm:
    ...
    if not pcm_chunks:
        return None
    return b"".join(pcm_chunks), sample_rate or 24000


@router.post("/tutorial_tts_audio")
async def tutorial_tts_audio(request: Request):
    try:
        data = await request.json()
        text = (data.get("text") or "").strip()
        if not text:
            return {"success": False, "error": "empty text"}
        text = re.sub(r"N\\s*\\.?\\s*E\\s*\\.?\\s*K\\s*\\.?\\s*O\\s*\\.?", "neko", text, flags=re.I)
        ...
        # Windows 本地语音优先（教程用最快可用的本地兜底）
        if os.name == "nt":
            sapi_result = _synthesize_windows_sapi_pcm(text)
            if sapi_result:
                pcm_bytes, sample_rate, sapi_err = sapi_result
                if pcm_bytes and len(pcm_bytes) > 0 and sample_rate:
                ...
                if sapi_err:
                    errors.append(sapi_err)

        if engine == "cosyvoice_bistream":
            _ensure_cosyvoice_process(ws_url)
            result = await _synthesize_local_cosyvoice_pcm(text, ws_url)
            if not result:
                errors.append("cosyvoice websocket failed")
        elif engine == "local_cosyvoice":
            result = await _synthesize_openai_bistream_pcm(text, ws_url, voice_id)
            if not result:
                errors.append("local tts websocket failed")
        elif engine == "gptsovits_v3":
            ws_url = info.get("ws_url") or ""
            result = await _synthesize_gptsovits_v3_pcm(text, ws_url, voice_id or "_default")
            if not result:
                errors.append("gptsovits v3 failed")
                # fallback to local CosyVoice
                fallback_ws = "ws://127.0.0.1:50000/api/v1/ws/cosyvoice"
                _ensure_cosyvoice_process(fallback_ws)
                result = await _synthesize_local_cosyvoice_pcm(text, fallback_ws)
                if not result:
                    errors.append("cosyvoice fallback failed")

        if not result:
            return {"success": False, "error": "; ".join(errors) or "local tts failed or not running"}
        ...
        logger.error(f"tutorial_tts_audio failed: {e}")
